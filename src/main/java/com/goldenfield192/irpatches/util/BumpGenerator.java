...
public class BumpGenerator {
    ...
    public static float evaluate(float percent, float scale){
        //roughly within [-1, 1]
        return noise.noise(percent, scale) * 2;
    }

    private static class PerlinNoise{
        private static final int[] perm = new int[] { 151, 160, 137, 91, 90, 15, 131, 13, 201,
        ...
        public float noise(float x, float scale) {
            x *= scale;
            int X = (int)Math.floor(x) & 255;
            x -= (float) Math.floor(x);
            float u = fade(x);
            return lerp(u, grad(perm[X], x),
                        grad(perm[X+1], x-1));
        }
